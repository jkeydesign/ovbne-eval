    ...
    function IntroScreen({ mode, onStart }) {
      ...
      const cautions = isVR ? [
        '시각체계(자연성·조화성·정교성) 내용을 꼭 숙지해주시고 점수를 평가해 주세요.',
        '평가 시안은 27개 이며 5점 리커트 척도를 체크해주세요.',
        '평가항목별 순차 평정 방식 : 자연성, 조화성, 정교성 3개를 한번에 평가 하지 마시고, 자연성의 의미를 기억하시면서 27개 전체 시안을 자연성만 먼저 평가 해주세요. 그 다음 조화성, 그 다음 정교성 순으로 평가항목별로 평가하시면 더 신속하고 정확합니다.'
      ] : [
            ...
            <div className="mx-auto grid max-w-[820px] gap-[34px] text-[16px] leading-[1.85] text-gray-800">
              <section className="bg-slate-50/50 border border-slate-200/80 rounded-2xl p-6 md:p-8 space-y-4 text-base font-medium text-slate-750" style={{ wordBreak: 'keep-all' }}>
                ...
                {isVR ? (
                  ...
                  <>
                    <p className="leading-relaxed">
                      따라서 본 예비평가는 AI가 생성한 48개의 로고 시안들 가운데 본실험에 사용할 수 있는 후보 시안 27개를 선별하기 위한 1차 평가 과정입니다.
                    </p>
                    <p className="leading-relaxed">
                    ...
                    <p className="leading-relaxed">
                      전문가님의 판단은 AI 시대에 전문 디자이너의 판단 기준과 역할을 이해하는 데 중요한 자료가 됩니다.
                    </p>
                  </>
                )}
                ...
                <p className="pt-2 border-t border-slate-100">
                  ※ 전문가 자격 확인 절차 후 사례비 혹은 상품권이 지급됩니다.
                </p>
              </section>
              ...
              <main className="mx-auto max-w-[820px] text-base font-medium leading-[1.85] text-slate-800 space-y-8" style={{ wordBreak: 'keep-all' }}>
                ...
                <section className="bg-slate-50/50 border border-slate-200/80 rounded-2xl p-6 md:p-8 space-y-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <p>
                      각 시안이 로고 후보로서 어떤 시각적 특성을 보이는지<br />
                      자연성, 조화성, 정교성의 세 기준에 따라 전문가의 시선으로 평가해 주세요.
                    </p>
                  </div>
                </section>
    ...
    function DraggableLogoCard({ s, onDragStart }) {
      ...
      return (
        <div
          ...
          className="relative cursor-grab active:cursor-grabbing bg-white p-2 border border-slate-200 rounded-lg hover:border-slate-400 hover:shadow-sm transition-all duration-200 flex flex-col items-center justify-center w-20 h-24 select-none shrink-0"
          title={`${s.candidateId || s.stimulusId} (시각체계 평균: ${s.visualMean.toFixed(2)})`}
        >
          <div className="w-16 h-16 flex items-center justify-center overflow-hidden bg-slate-50/50 rounded-md">
          ...
          <span className="text-[10px] font-black font-mono text-slate-800 mt-1 tracking-tight">{s.candidateId || s.stimulusId}</span>
        </div>
      );
    }
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="grid grid-cols-3 gap-2">
                                {/* Type A Column */}
                                <div className="bg-white p-2 rounded-lg border border-slate-200 shadow-xs flex flex-col min-h-[340px]">
                          ...
                          <tbody className="divide-y divide-slate-100 text-slate-700">
                            {candidateStats.map((s, idx) => {
                              ...
                              return (
                                <tr key={idx} className="hover:bg-slate-50">
                                  <td className="p-3">
                                    <div className="w-16 h-16 border border-slate-200 rounded-lg bg-white flex items-center justify-center overflow-hidden shadow-xs">
                      ...
                      <tbody className="divide-y divide-slate-100 text-slate-700">
                        {(selectedSubmission.ratings || []).map((r, idx) => {
                          ...
                          return (
                            <tr key={idx} className="hover:bg-slate-50">
                              <td className="p-2">
                                <div className="w-14 h-14 border border-slate-100 rounded bg-white flex items-center justify-center overflow-hidden">
